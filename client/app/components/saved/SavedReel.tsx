...
const SavedReel = ({postId, isClicked, setIsClicked} : {postId : any, isClicked : any, setIsClicked : any}) => {
  ...
  const unsave = async() => {
    try {
      const resp = await httpClient.post("http://127.0.0.1:5000/otherUnsave", {
        postId,
      });
    }
    catch (error : any){
      ...
      <View style={styles.cardBody}>
        <View style = {{alignItems : "center"}}>
          <TouchableOpacity onPress={() =>Linking.openURL(url)}>
            <Image style = {{width : 300, height:310, borderRadius : 12}} source={{uri :photo}}></Image>
          </TouchableOpacity>
